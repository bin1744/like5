      ...
      <div id="sidebar">
        ...
        <c:choose>
         ...
         <div id="sidmenu">  
          ...
          <div id="" onclick="location.href='donate.me?memNo=${ loginUser.memNo }'"  style="color:white; background-color:rgb(220, 53, 69);">
                  	후원
          </div>
                ...
                <div>
                    ...
                    <tbody>
                      <c:forEach var="list" items="${ list }">
	                    	<c:choose>
	                    		<c:when test="${ list.sponsorship.sponCategory eq '1'}">
	                    			<tr onclick="location.href='qnaDetail.bo?bno=${ list.sponsorship.refBoaNo }'">
	                    				<td><fmt:formatDate pattern="yy/MM/dd" value="${ list.sponsorship.sponDate }"/></td>
						                <td>${ list.member.nickName }</td>
						                <td>QNA</td>
						          	</tr>
	                    		</c:when>
	                    		<c:when test="${ list.sponsorship.sponCategory eq '2' }">
	                    			<tr onclick="location.href='colDetail.bo?bno=${ list.sponsorship.refBoaNo }'">
	                    				<td><fmt:formatDate pattern="yy/MM/dd" value="${ list.sponsorship.sponDate }"/></td>
						                <td>${ list.member.nickName }</td>
						                <td>칼럼</td>
						          	</tr>
                  ...
                  <div id="pagingArea">
	                	<ul class="pagination">
			                    ...
			                    <c:otherwise>
			                    	<c:choose>
			                    		<c:when test="${ !empty condition }">
			                    			<li class="page-item"><a class="page-link" href="memDonatesearch.me?memNo=${loginUser.memNo}&dcurrentPage=${ pi.currentPage-1 }&condition=${condition}&keyword=${keyword}"><</a></li>
		                    			</c:when>
		                    			<c:otherwise>
		                    				<li class="page-item"><a class="page-link" href="memDonate.me?memNo=${loginUser.memNo}&currentPage=${ pi.currentPage-1 }"><</a></li>
		                    			</c:otherwise>
		                    		</c:choose>
		                    	</c:otherwise>
		                    </c:choose>
		                    ...
		                    <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
		                    	<c:choose>
		                    		<c:when test="${ !empty condition }">
		                   				<li class="page-item"><a class="page-link" href="memDonatesearch.me?memNo=${loginUser.memNo}&currentPage=${ p }&condition=${condition}">${ p }</a></li>
		                   			</c:when>
		                   			<c:otherwise>
		                   				<li class="page-item"><a class="page-link" href="memDonate.me?memNo=${loginUser.memNo}&currentPage=${ p }">${ p }</a></li>
		                   			</c:otherwise>
		                    	</c:choose>
		                    </c:forEach>
		                    ...
		                    <c:choose>
			                    ...
			                    <c:otherwise>
			                    	<c:choose>
			                    		<c:when test="${ !empty condition }">
				                    		<li class="page-item"><a class="page-link" href="memDonatesearch.me?memNo=${loginUser.memNo}&currentPage=${ pi.currentPage+1 }&condition=${condition}">></a></li>
				                    	</c:when>
				                    	<c:otherwise>
				                    		<li class="page-item"><a class="page-link" href="memDonate.me?memNo=${loginUser.memNo}&currentPage=${ pi.currentPage+1 }">></a></li>
				                    	</c:otherwise>
